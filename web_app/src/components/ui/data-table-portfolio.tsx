...
import { formatCompactNumber, formatAddress, formatDate, getTokenName, getTokenExplorer } from '@/lib/utils';
...
export function DataTable<TData, TValue>({ columns, data, userSearchColumn, inputPlaceHolder }: DataTableProps<PortfolioTableDataType>) {
    ...
    const renderCellContent = (cell: CellContext<PortfolioTableDataType, unknown>, row: { original: PortfolioTableDataType }) => {
        switch (cell.column.id) {
            ...
            case 'mode':
                return (
                    <Badge className={row.original.mode === 'Buy' ? 'bg-primary' : 'bg-[#FF0066] hover:bg-[#f64189]'}>
                        {row.original.mode === 'Buy' ? 'Mint' : 'Sell'}
                    </Badge>
                );
            case 'tickIn':
                return (
                    <div className='flex flex-row space-x-1 items-center'>
                        <div>{formatCompactNumber(parseFloat(row.original.tickIn))}</div>
                        <div>{getTokenName(row.original.token_in_address)}</div>
                    </div>
                );
            case 'tickOutName':
                return (
                    <div className='flex flex-row space-x-1 items-center'>
                        <div>{formatCompactNumber(parseFloat(row.original.token_out_amount))}</div>
                        <div>{getTokenName(row.original.token_out_address)}</div>
                    </div>
                );
            case 'tokenBoughtAt':
                return formatDate(row.original.transaction_timestamp);
            case 'viewTransaction':
                return (
                    <a
                        href={`${getTokenExplorer(row.original.token_out_address)}${getTokenExplorer(row.original.token_out_address).endsWith('/') ? '' : '/'}${row.original.viewTransaction}`}
                        target='_blank'
